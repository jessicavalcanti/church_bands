...
// Ajuste local (#87): `pointer-coarse:opacity-100` no fim. O resto é como o
// SaladUI escreveu — o `x` nasce transparente e aparece com o mouse em cima do
// cartão, que é o que o Sonner faz. Onde não há mouse para pousar, essa regra
// nunca valeria e o botão ficaria invisível para sempre; no dedo ele fica
// sempre à vista.
const CLOSE_TRIGGER_CLASSES =
  ...
  "focus-visible:outline-hidden focus-visible:ring-ring/50 focus-visible:ring-[3px] pointer-coarse:opacity-100";
...
class ToastComponent extends Component {
  ...
  getOrCreateViewport(position) {
    ...
    // Ajuste local (#87): a interface fala português, e o leitor de tela lê
    // este rótulo em voz alta.
    viewport.setAttribute("aria-label", "Avisos");
  ...
  populateCard(card, id, params, variant, position) {
    ...
    // Ajuste local (#87): o `x` do cartão. `buildCloseTrigger` está escrito
    // logo abaixo desde o v1.0 do SaladUI — a nota acima chega a dizer que o
    // handler de fechar é reatado aqui —, mas nada o chamava, e nenhum toast
    // desenhava o botão. Só cartão que se pode dispensar ganha o seu: o
    // `dismissible: false` diz que aquele toast fecha por outra coisa.
    if (dismissible) {
      card.appendChild(this.buildCloseTrigger(id));
  ...
  buildCloseTrigger(id) {
    ...
    // Ajuste local (#87): a interface fala português.

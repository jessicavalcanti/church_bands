defmodule ChurchBandsWeb.PortalTest do
  ...
  describe "itens do menu" do
    test "músico comum vê Início, Bandas, Pessoas, Calendário e Trocas — e nada do que é de acesso total",
         %{conn: conn} do
    ...
    test "o menu segue a ordem Início, Bandas, Músicas, Pessoas, Instrumentos, Calendário, Trocas, Tipos, Convites",
         %{conn: conn} do
      ...
      posicoes =
        Enum.map(
          ~w(home-link bands-link songs-link users-link instruments-link calendar-link
             swaps-link event-types-link invites-link),
  ...
  describe "a dica do item do menu" do
    ...
    test "nenhum item do menu mostra a dica enquanto a barra está aberta", %{conn: conn} do
      ...
      assert length(dicas) == 9, "são nove itens de menu para o Pastor"

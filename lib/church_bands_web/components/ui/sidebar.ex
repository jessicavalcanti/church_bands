defmodule ChurchBandsWeb.Components.UI.Sidebar do
  @moduledoc false
  ...
  def sidebar_trigger(assigns) do
    # Ajuste local (US 1.9): `variant` e `size` só fazem sentido para o
    # componente de botão. Com o `as` padrão (`"button"`) eles iam parar no
    # HTML como atributos inventados; aqui viram as classes correspondentes.
    ~H"""
    <.dynamic
      tag={@as}
      type="button"
      data-sidebar="trigger"
      class={classes([button_variant(%{variant: "ghost", size: "icon"}), "h-7 w-7", @class])}
  ...
  # Ajuste local (US 1.9): o item do menu do portal precisa navegar por
  # LiveView. Com `navigate`, `patch` ou `href` o botão vira `<.link>`, no mesmo
  # espírito do `button/1` do Phoenix; sem eles, nada muda.
  def sidebar_menu_button(%{rest: rest} = assigns)
      when is_map_key(rest, :navigate) or is_map_key(rest, :patch) or is_map_key(rest, :href) do
    button = ~H"""
    <.link
